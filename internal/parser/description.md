...
Тут реализовано базовое AST-дерево.

![img.png](../../docs/img.png)

В нашем случае математическое выражение может состоять из: 

1. цифры 0..9
2. операторы + - * / 

Операторы имеют приоритет (чем больше - тем приоритетнее):
1. "+" и "-" имеют приоритет 11
2. "/" и "*" имеют приоритет 12

Приоритеты взял отсюда, дабы в дальнейшем было куда расшириться :)
[Приоритет операторов - JavaScript](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Operators/Operator_Precedence)

При этом унарных операций не предполагается.

У каждого бинарного оператора есть что-то слева и что-то справа.

Приоритеты позволяют сформировать точки перегиба для построения AST-дерева.
Изначально всё выражение полностью токенизируется. После этого необходимо посмотреть, где у нас есть точка перегиба. Точка перегиба арифметического выражения - который является оператором (арифметическим действием) и имеет максимальное значение приоритета по отношению к другим операторам.
В примере `1+2-3+1/2` точкой перегиба станет оператор `/`. Она разделит математическое выражение для этого оператора на `1+2-3+1` слева и `2` справа.




Для того чтобы в дальнейшем разделить математическое выражение на части для параллельного и независимого вычисления, да и в целом, вычислить выражение, нам требуется токенизировать каждый символ - присваивать ему определенный токен. То есть будет всего 2 токена:
* NumberToken
* OperatorToken

Чтобы разделить выражение на обособленные части, необходимо удостовериться, что оно удовлетворяет этим условиям:
1. обособленная часть . Либо, 

Как понять, что 

Оно должно делать это:

1. **Токенизация**. Посимвольное чтение ввода, чтобы каждому символу присвоить токен. 